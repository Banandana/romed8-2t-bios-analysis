...
**Per-slot scope is real in the IFR for AMD PCIE Link Speed.** Each PCIe physical slot has its own NVRAM offset in the standard `Setup` UEFI variable. The 11 entries are at distinct, contiguous offsets `0x123–0x12D` (1 byte each), with no shared backing storage.
...
The AMD CBS / NBIO Common Options menu is real, but does **not** contain any per-slot Target Link Speed option in P3.70.
